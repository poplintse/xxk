...
构建完成后会自动运行 `scripts/macos/verify_release.sh`，检查归档结构、版本元数据、arm64 架构、App Sandbox、Hardened Runtime、签名和校验和。
...
```bash
./scripts/macos/publish_test_release.sh
```
...
```bash
SIGNING_IDENTITY="Developer ID Application: …" ./scripts/macos/build_release.sh
```
...
```bash
NOTARY_PROFILE="xuxiake-notary" ./scripts/macos/notarize.sh
```
