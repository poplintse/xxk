...
VERSION_FILE="$MACOS_DIR/VERSION"
BUILD_NUMBER_FILE="$MACOS_DIR/BUILD_NUMBER"
APP_BUNDLE="${2:-$REPO_ROOT/dist/$APP_NAME.app}"
APP_CONTENTS="$APP_BUNDLE/Contents"
APP_MACOS="$APP_CONTENTS/MacOS"
APP_RESOURCES="$APP_CONTENTS/Resources"
APP_BINARY="$APP_MACOS/$APP_NAME"
INFO_PLIST="$APP_CONTENTS/Info.plist"
ASSET_INFO="$MACOS_DIR/.build/asset-info.plist"
...
APP_VERSION="${APP_VERSION_OVERRIDE:-$(tr -d '[:space:]' <"$VERSION_FILE")}"
APP_BUILD_NUMBER="${APP_BUILD_NUMBER_OVERRIDE:-$(tr -d '[:space:]' <"$BUILD_NUMBER_FILE")}"
...
case "$APP_BUNDLE" in
  "$REPO_ROOT"/dist/*.app|"$REPO_ROOT"/dist/*/*.app|"$REPO_ROOT"/artifacts/macos/*/*.app) ;;
  *)
    echo "output app must be inside $REPO_ROOT/dist or $REPO_ROOT/artifacts/macos and end in .app" >&2
    exit 2
    ;;
esac
...
BUILD_ARGUMENTS=(--package-path "$MACOS_DIR" --configuration "$CONFIGURATION")
