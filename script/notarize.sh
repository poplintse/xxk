...
APP_BUNDLE="$ROOT_DIR/dist/release/XuXiake.app"
ARCHIVE="$ROOT_DIR/dist/release/XuXiake.zip"
...
if [[ ! -f "$ARCHIVE" || ! -f "$CHECKSUM" ]]; then
  echo "Release archive or checksum not found; run script/build_release.sh first" >&2
  exit 2
fi
